...
export function RatingStars({ rating }: { rating: number | undefined }) {
    if (!rating) {
        rating = 0
    }
    return (
        <div className="w-[155px] flex gap-3 justify-between items-center" style={{ userSelect: "none" }}>
            <span className="text-[20px] montseratt font-semibold" style={{ userSelect: "none" }}>{rating.toFixed(1)}</span>
